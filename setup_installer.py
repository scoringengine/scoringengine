...
def run_bootstrap_once():
    ...
    code, out = run_cmd(
        ["docker", "compose", "up", "--no-deps", "--abort-on-container-exit", "bootstrap"], check=False, capture=True
    )
    if code != 0:
        print(out)
...
def write_compose_override():
    """Generate docker-compose.override.yml to mount the installer-generated config file.

    This keeps the base docker-compose.yml working for users who haven't run the
    installer, while giving installer users their custom config automatically.
    """
    override_path = CONFIG_DIR / "docker-compose.override.yml"
    content = """\
...
def parse_args():
    ...
    p.add_argument(
        "--non-interactive", action="store_true", help="Read inputs from env vars and write config files only."
    )
...
This interactive installer will:
  ...
  - generate docker/engine.conf.inc for the Scoring Engine
...
""")
    ...
    r_ok, r_err = test_redis_inside_docker(
        config["redis"]["redis_host"], config["redis"]["redis_port"], config["redis"].get("redis_password", "")
    )
    if not r_ok:
        print("\nRedis test failed.")
        print(r_err)
