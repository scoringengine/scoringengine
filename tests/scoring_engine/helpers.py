...
def generate_sample_model_tree(model, session):
    ...
    # Users
    user = User(
        username="testuser" + str(random.randrange(10000)), password="catdog", team=team
    )
    session.add(user)
    session.commit()
    if model == "User":
    ...
    # Services
    service = Service(
        name="ICMP IPv4", team=team, check_name="ICMP IPv4 Check", host="127.0.0.1"
    )
    session.add(service)
    session.commit()
    if model == "Service":
    ...
    property_obj = Property(
        name="testproperty", value="testvalue", environment=environment
    )
