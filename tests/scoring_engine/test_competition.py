...
class CompetitionDataTest:
    def _init_competition_data(self):
        """Initialize the competition setup data dict."""
        self.competition_setup = {
            "teams": [
                {
                    "name": "Team1",
                    "color": "Blue",
                    "users": [{"username": "team1user1", "password": "testpass"}],
                    "services": [
                        {
                            "name": "SSH",
                            "check_name": "SSHCheck",
                            ...
                            "points": 150,
                            "accounts": [
                                {"username": "ttesterson", "password": "testpass"},
                                {"username": "rpeterson", "password": "redfred"},
                            ],
                            "environments": [
                                {
                                    "matching_content": "^SUCCESS",
                                    "properties": [
                                        {"name": "commands", "value": "id;ls -l"},
                                    ],
                                },
                                {
                ...
                },
                {
                    "name": "WhiteTeam",
                    "color": "White",
                    "users": [{"username": "whiteteamuser", "password": "testpass"}],
                },
                {"name": "RedTeam", "color": "Red", "users": [{"username": "redteamuser", "password": "testpass"}]},
            ],
...
class TestTeamsData(CompetitionDataTest):
    ...
    def test_team_missing_service(self):
        ...
        second_team = copy.deepcopy(self.competition_setup["teams"][0])
        second_team["name"] = "Team 2"
        second_team["services"][0]["name"] = "OtherService"
        self.competition_setup["teams"].append(second_team)
    ...
    def test_team_different_service_check_name(self):
        ...
        self.competition_setup["teams"].append(second_team)
        self.verify_error(
            "Incorrect check_name for Service 'SSH' for Team 'Team 2'. Got: 'ICMPCheck' Expected: SSHCheck"
        )
...
class TestUserData(CompetitionDataTest):
    def test_no_username(self):
        del self.competition_setup["teams"][0]["users"][0]["username"]
        self.verify_error("Team1 user must have a 'username' field")

    def test_user_bad_username(self):
        self.competition_setup["teams"][0]["users"][0]["username"] = []
        self.verify_error("Team1 user username must a string")

    def test_no_password(self):
        del self.competition_setup["teams"][0]["users"][0]["password"]
...
class TestPropertyData(CompetitionDataTest):
    def test_no_name(self):
        del self.competition_setup["teams"][0]["services"][0]["environments"][0]["properties"][0]["name"]
        self.verify_error("Team1 SSH property must have a 'name' field")

    def test_bad_name_type(self):
        self.competition_setup["teams"][0]["services"][0]["environments"][0]["properties"][0]["name"] = []
        self.verify_error("Team1 SSH property 'name' field must be a string")

    def test_no_value(self):
        del self.competition_setup["teams"][0]["services"][0]["environments"][0]["properties"][0]["value"]
        self.verify_error("Team1 SSH property must have a 'value' field")

    def test_bad_value_type(self):
        self.competition_setup["teams"][0]["services"][0]["environments"][0]["properties"][0]["value"] = []
        self.verify_error("Team1 SSH property 'value' field must be a string")

    def test_bad_name_value(self):
        self.competition_setup["teams"][0]["services"][0]["environments"][0]["properties"][0]["name"] = "someproperty"
        self.verify_error("Team1 SSH SSHCheck does not require the property 'someproperty'")

    def test_all_required_properties(self):
        del self.competition_setup["teams"][0]["services"][0]["environments"][0]["properties"][0]
...
class TestGoodSetup(CompetitionDataTest):
    ...
    def test_teams(self):
        assert len(db.session.query(Team).all()) == 3
