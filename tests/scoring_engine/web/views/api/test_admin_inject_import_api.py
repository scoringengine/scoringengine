...
class TestAdminInjectImportAPI(UnitTest):
    ...
    def setup_method(self):
        ...
        self.session.add_all([self.white_team, self.blue_team1, self.blue_team2, self.red_team])
        self.session.commit()

        self.white_user = User(username="whiteuser", password="pass", team=self.white_team)
        self.blue_user = User(username="blueuser", password="pass", team=self.blue_team1)
    ...
    def test_import_with_nonexistent_id_returns_error(self):
        """Importing with an id that doesn't exist in the DB should return 400"""
        self.login("whiteuser", "pass")
        ...
        assert resp.json["message"] == "Invalid Template ID"
