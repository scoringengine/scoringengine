...
class TestJobQueue(object):
    ...
    def compare_jobs(self, src_job, dst_job):
        ...
        assert src_job.environment_id == dst_job.environment_id
    ...
    def test_clear(self):
        job = Job(environment_id="102938", command="ls -l")
    ...
    def test_add_job(self):
        job = Job(environment_id="12345", command="ls -l")
        self.queue.add_job(job)
        assert self.queue.size() == 1
        found_job = self.queue.get_job()
    ...
    def test_adding_multiple_jobs(self):
        job_1 = Job(environment_id="12345", command="ping -c 127.0.0.1")
        job_2 = Job(environment_id="54321", command="ping -c 127.0.0.1")
