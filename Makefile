...
integration-get-round:
	docker compose -f $(MAIN_DOCKER) -f $(TESTBED_DOCKER) -f $(INTEGRATION_DOCKER) -p $(PROJECT_NAME) run --rm --no-deps engine bash -c "python -c 'from scoring_engine.web import create_app; from scoring_engine.models.round import Round; app = create_app(); app.app_context().push(); print(\"(Round {0})\".format(Round.get_last_round_num()))'"
