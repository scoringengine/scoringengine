...
class CeleryStats:
    ...
    def get_queue_stats():
        ...
        for service in all_services:
            ...
            if (
                service.team.name
                not in queues_facts[service.worker_queue]["services_running"]
            ):
        ...
        for queue_name, queue_facts in queues_facts.items():
            ...
            for team_name, team_services in queues_facts[service.worker_queue][
                "services_running"
            ].items():
            ...
            if queue_services_total_running == len(all_services):
                ...
                for blue_team in blue_teams:
                    if blue_team.name in queue_facts["services_running"] and len(
                        blue_team.services
                    ) == len(queue_facts["services_running"][blue_team.name]):
            ...
            if worker_services_total_running == len(all_services):
                ...
                for blue_team in blue_teams:
                    if blue_team.name in services_running and len(
                        blue_team.services
                    ) == len(services_running[blue_team.name]):
                        # Summarize it for each team if the worker runs all services
