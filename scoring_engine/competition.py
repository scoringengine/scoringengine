...
class Competition(dict):
    ...
    def verify_data(self, data):
        ...
        for team in data["teams"]:
            for user in team["users"]:
                assert user["username"] not in usernames, "Multiple Users with the same username: '{0}'".format(
                    user["username"]
                )
    ...
    def verify_team_data(self, team):
        ...
        assert team["color"] in ("Blue", "White", "Red"), "'{0}' color must one of (Red, White, Blue)".format(
            team["name"]
        )
        ...
        if team["color"] == "Blue":
            ...
            for required_service in self.required_services:
                ...
                assert team_service is not None, "Service '{0}' not defined in team '{1}'".format(
                ...
                assert (
                    ...
                    team_service["name"], team["name"], team_service["check_name"], required_service["check_name"]
                )
                assert (
                    team_service["points"] == required_service["points"]
                ), "Incorrect points for Service '{0}' for Team '{1}'. Got: {2} Expected: {3}".format(
                    team_service["name"], team["name"], team_service["points"], required_service["points"]
                )
            ...
            for team_service in team["services"]:
                ...
                assert required_service is not None, "Service '{0}' for Team '{1}' not defined in other teams".format(
    ...
    def verify_service_data(self, service, team_name):
        ...
        assert "check_name" in service, "{0} {1} service must have a 'check_name' field".format(
            team_name, service["name"]
        )
        assert type(service["check_name"]) is str, "{0} {1} service 'check_name' field must be a string".format(
            team_name, service["name"]
        )
        ...
        assert type(service["host"]) is str, "{0} {1} service 'host' field must be a string".format(
            team_name, service["name"]
        )

        # Verify service worker_queue if it exists
        if "worker_queue" in service:
            assert type(service["worker_queue"]) is str, "{0} {1} service 'worker_queue' field must be a string".format(
                team_name, service["name"]
            )

        # Verify service port
        assert "port" in service, "{0} {1} service must have a 'port' field".format(team_name, service["name"])
        assert type(service["port"]) is int, "{0} {1} service 'port' field must be an integer".format(
            team_name, service["name"]
        )

        # Verify service points
        assert "points" in service, "{0} {1} service must have a 'points' field".format(team_name, service["name"])
        assert type(service["points"]) is int, "{0} {1} service 'points' field must be an integer".format(
            team_name, service["name"]
        )

        if "accounts" in service:
            assert type(service["accounts"]) is list, "{0} {1} service 'accounts' field must be an array".format(
                team_name, service["name"]
            )
        ...
        assert "environments" in service, "{0} {1} service must have a 'environments' field".format(
            team_name, service["name"]
        )
        assert type(service["environments"]) is list, "{0} {1} service 'environments' field must be an array".format(
            team_name, service["name"]
        )
    ...
    def verify_account_data(self, account, team_name, service_name):
        ...
        assert type(account["username"]) is str, "{0} {1} account 'username' field must be a string".format(
        ...
        assert type(account["password"]) is str, "{0} {1} account 'password' field must be a string".format(
            team_name, service_name
        )
    ...
    def verify_environment_data(self, environment, team_name, service_name, found_check_source):
        ...
        assert "matching_content" in environment, "{0} {1} environment must have a 'matching_content' field".format(
            team_name, service_name
        )
        assert (
            type(environment["matching_content"]) is str
        ), "{0} {1} environment 'matching_content' field must be a string".format(team_name, service_name)
        try:
            re.compile(environment["matching_content"])
        except re.error as e:
            assert False, "{0} {1} environment 'matching_content' field must be a valid regex pattern: {2}".format(
        ...
        if "properties" in environment:
            ...
            ), "{0} {1} environment 'properties' field must be an array".format(team_name, service_name)
            ...
            for required_property_key in found_check_source.required_properties:
                ...
                for defined_property in environment["properties"]:
                    if required_property_key in defined_property["name"]:
                        matched_key = True
                assert matched_key is True, "{0} {1} service does not define the '{2}' property".format(
    ...
    def verify_property_data(self, property_obj, team_name, service_name, found_check_source):
        ...
        assert type(property_obj["name"]) is str, "{0} {1} property 'name' field must be a string".format(
            team_name, service_name
        )

        # Verify property value
        assert "value" in property_obj, "{0} {1} property must have a 'value' field".format(team_name, service_name)
        assert type(property_obj["value"]) is str, "{0} {1} property 'value' field must be a string".format(
            team_name, service_name
        )
        assert (
    ...
    def save(self):
        ...
        for flag in self["flags"]:
            ...
            if not start:
                ...
                )  # TODO - This is hacky, find a better way to fix this
            if not end:
                end = str(
